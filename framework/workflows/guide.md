...
1.  **Understand the Past (Historical Context Assessment - HCA)**
    ...
    *   **Output Template:** `templates/historical-context-report.md`
...
2.  **Define "Fairness" (Fairness Definition Selection - FDS)**
    ...
    *   **Output Template:** `templates/fairness-definition-rationale.md`
...
3.  **Find Potential Problems (Bias Source Identification - BSI)**
    ...
    *   **Output Template:** `templates/bias-source-mapping.md`
...
4.  **Measure the Results (Metrics Implementation & Analysis - MIA)**
    ...
    *   **Output Templates:** `templates/metrics-implementation.md`, `templates/intersectional-analysis.md`
...
1.  **Plan**: Complete `templates/assessment-plan.md`.
2.  **Assess Historical Context**: Research and document findings in `templates/historical-context-report.md`.
3.  **Select Fairness Definitions**: Analyze options, justify choices in `templates/fairness-definition-rationale.md`.
4.  **Identify Bias Sources**: Analyze the lifecycle, assess risks, document in `templates/bias-source-mapping.md`.
5.  **Implement Metrics & Analyze**: Calculate metrics, perform disaggregated/intersectional analysis, document in `templates/metrics-implementation.md` and `templates/intersectional-analysis.md`.
6.  **Synthesize & Report**: Analyze connections across components, document limitations (`templates/limitations-acknowledgment.md`), record stakeholder input (`templates/stakeholder-documentation.md`), and summarize findings (`templates/executive-summary.md`, potentially `templates/model-card.md`).

## 6. Essential Documentation: Using Templates

Consistent documentation is key. Use the provided Markdown templates in the `templates/` directory for the outputs of each step defined in your chosen workflow. Key templates include:
...
*   **Fairness Effectiveness:**
    ...
    *   Does it analyze intersectionality appropriately? (See `resources/stakeholder-intersectionality.md`)
...
*   **Usability & Process:**
    ...
    *   Was stakeholder input appropriately gathered and integrated? (See `templates/stakeholder-documentation.md`)
    *   Was the process efficient for the chosen scope?
*   **Honesty (Limitations):**
    *   Are the limitations (data, scope, methods) clearly documented? (See `templates/limitations-acknowledgment.md`)
...
*   **Documentation**:
    ...
    *   Maintain a change log (e.g., in `assessment-plan.md` or `executive-summary.md`) detailing changes between versions, the author, date, reason, and impact.
    *   Note the overall *Framework Version* (if the framework itself is versioned) that the assessment was based on in the `assessment-plan.md`.
...
*   **Domain/Regulatory Adaptation**: See `resources/adaptation-compliance.md`. Tailor the focus based on industry (hiring, finance, health) and legal requirements (EU AI Act, EEOC, etc.).
*   **Stakeholder Engagement & Intersectionality**: See `resources/stakeholder-intersectionality.md` for methods on involving people effectively and analyzing overlapping identities.
*   **(Optional) Causal Fairness**: For deeper root cause analysis, see `resources/causal-fairness.md`.
